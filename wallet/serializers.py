...
class FundWalletSerializer(serializers.ModelSerializer):
    ...
    amount = serializers.FloatField(min_value=100.0, max_value=99999.99, required=True)
...
class TransferWalledSerializer(serializers.Serializer):
    ...
    reciever = serializers.PrimaryKeyRelatedField(queryset=Wallet.objects.all(), required=True)
    amount = serializers.FloatField(min_value=100.0, max_value=45000.0, required=True)
